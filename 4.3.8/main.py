    ...
    def insert(self, value):
        ...
        while True:
            ...
            if parent_index >= 0 and self.queue[parent_index] < self.queue[value_index]:
                self.queue[value_index], self.queue[parent_index] = self.queue[parent_index], self.queue[value_index]
        ...
        if shift_node == 0:
            ...
            while True:
                ...
                if 0 <= left_child < n and 0 <= right_child < n:
                    if self.queue[shift_node] >= self.queue[left_child] and self.queue[shift_node] >= self.queue[right_child]:
                        break
                    if self.queue[left_child] < self.queue[right_child]:
                        self.queue[right_child], self.queue[shift_node] = self.queue[shift_node], self.queue[right_child]
                        shift_node = right_child
                        continue
                    else:
                        self.queue[left_child], self.queue[shift_node] = self.queue[shift_node], self.queue[left_child]
